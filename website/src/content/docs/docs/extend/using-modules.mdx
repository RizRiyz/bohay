...
`RizRiyz/bohay-agent-ping` calls itself `example.agent-ping` just to remove it:

```sh
bohay module uninstall RizRiyz/bohay-agent-ping   # the name you installed with
bohay module uninstall example.agent-ping         # the module's own id
```
