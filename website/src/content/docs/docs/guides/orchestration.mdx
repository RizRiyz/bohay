...
| **Worker** | A pane running an agent in a dedicated git worktree on its own branch (`bohay/<id>`) for physical isolation, so two workers literally edit different files on disk. |
...
`review` (gate failed, so fix and retry) → merged, or `blocked` on a merge
...
3. Select `t1` and press `s`. bohay creates a worktree on branch `bohay/t1`,
...
6. Select a done task, `m`, and the branch merges into `bohay/integration` in an
