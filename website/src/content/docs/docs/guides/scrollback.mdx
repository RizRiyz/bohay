...
scrolling do too. Regular transcripts scroll Bohay's retained history.

Each pane has a **10 MiB** retained-history budget by default. Change it in
**Settings → Layout → Scrollback memory**. Bohay's Alacritty adapter converts this
...
application-cursor pager), Bohay forwards those keys unchanged. `Shift+↑`,
`Shift+PageUp`, and prefix scroll shortcuts remain explicit Bohay controls.
...
all navigation inside Bohay: use arrows or `h`/`j`/`k`/`l` for character and
