...
switches the prefix to `Ctrl+b` and maps the tmux keys onto bohay:
...
bohay's defaults. A preset is a starting point, so you can still rebind
individual keys afterward. Choose **bohay (default)** to revert.

Note: `,` renames the tab on both the bohay defaults and the tmux preset (the
