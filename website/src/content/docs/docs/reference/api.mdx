...
declares the dock in its manifest and pushes its content here; bohay owns the
rendering.
...
A `Row` is `{"text": string, "dot"?: "idle"|"working"|"blocked"|"done",
"action"?: string}`. The first `push` mounts the dock into `placement` (default
left); later pushes refresh its rows. A row's `action` is a module action id,
run when the row is clicked. The CLI wrappers are `bohay ui dock push|list|move`
(`push` also reads its rows from stdin).
