...
```sh
luvus ui bar list
luvus ui bar push --id status --region top-right --content '[{"type":"text","text":"CI"}]'
luvus ui bar push --id status --content-file ./bar.json
luvus ui bar move --id status --region bottom-right
luvus ui bar remove --id status
