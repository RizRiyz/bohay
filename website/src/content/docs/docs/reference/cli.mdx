...
Use `bohay help <topic>` for one command area or append `--help` to a
specific command, such as `bohay pane split --help`. Explicit help writes to
stdout, while invalid usage writes to stderr.
