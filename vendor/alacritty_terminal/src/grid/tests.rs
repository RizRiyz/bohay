...
#[test]
fn scroll_up_compacts_cold_history_without_changing_cells() {
    let mut grid = Grid::<usize>::new(2, 8, 10);
    grid[Line(0)][Column(0)] = 11;
    grid[Line(0)][Column(1)] = 22;

    grid.scroll_up::<usize>(&(Line(0)..Line(2)), 1);

    assert_eq!(grid.history_size(), 1);
    assert_eq!(grid.compacted_history_rows(), 1);
    assert_eq!(grid[Line(-1)][Column(0)], 11);
    assert_eq!(grid[Line(-1)][Column(1)], 22);
    assert_eq!(grid[Line(-1)][Column(7)], 0);

    // Mutable access is uncommon for history but must remain lossless for
    // resize, reflow, and terminal editing operations.
    grid[Line(-1)][Column(7)] = 33;
    assert_eq!(grid.compacted_history_rows(), 0);
    assert_eq!(grid[Line(-1)][Column(0)], 11);
    assert_eq!(grid[Line(-1)][Column(7)], 33);
}
